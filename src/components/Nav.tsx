      ...
      <header className="fixed top-5 left-1/2 -translate-x-1/2 z-50 w-max max-w-[calc(100vw-2rem)]">
        ...
        <div className="rounded-full px-2 py-2 bg-white/[0.06] ring-1 ring-white/[0.10] backdrop-blur-xl shadow-[0_8px_40px_rgba(0,0,0,0.5)]">
          <div className="flex items-center gap-1">
            ...
            <nav className="hidden md:flex items-center gap-0.5">
              {links.map((link) => (
                <Link
                  key={link.label}
                  href={link.href}
                  className="px-4 py-2 rounded-full text-[11px] font-semibold tracking-[0.14em] text-white/55 hover:text-bone hover:bg-white/[0.07] transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)]"
                >
                  {link.label}
                </Link>
              ))}
            ...
            <Link
              href="#events"
              className="hidden md:flex items-center gap-1.5 pl-5 pr-1.5 py-1.5 rounded-full bg-accent text-white text-[11px] font-semibold tracking-wide hover:bg-accent-light active:scale-[0.98] transition-all duration-300 ease-[cubic-bezier(0.32,0.72,0,1)] group ml-1"
            >
            ...
            <button
              className="md:hidden w-9 h-9 rounded-full flex items-center justify-center text-white/70 hover:text-bone hover:bg-white/[0.07] transition-all duration-300 ml-1"
              onClick={() => setOpen(!open)}
              aria-label={open ? "Close menu" : "Open menu"}
            >
              <div className="relative w-5 h-[14px]">
                <span
                  className={`absolute left-0 top-0 h-[1.5px] w-5 bg-current rounded-full transition-all duration-400 ease-[cubic-bezier(0.32,0.72,0,1)] origin-center ${
                    open ? "top-[6px] rotate-45" : ""
                  }`}
                />
                <span
                  className={`absolute left-0 bottom-0 h-[1.5px] w-5 bg-current rounded-full transition-all duration-400 ease-[cubic-bezier(0.32,0.72,0,1)] origin-center ${
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed inset-0 z-40 backdrop-blur-3xl bg-[#090e09]/85 flex flex-col items-center justify-center"
          >
